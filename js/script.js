...
// =====================================================
// SIH Internals 2026 Popup — Fully self-contained.
// Shows automatically once per browser session.
// Placed OUTSIDE the main DOMContentLoaded block so that
// errors in the main script cannot prevent this popup.
// =====================================================
document.addEventListener('DOMContentLoaded', function () {
    var SIH_SESSION_KEY = 'sihInternals2026PopupSeen';
    var popup   = document.getElementById('sihInternalsPopup');
    var closeBtn = document.getElementById('sihPopupClose');

    // Guard — element must exist & not seen this session
    if (!popup) return;
    if (sessionStorage.getItem(SIH_SESSION_KEY)) return;

    /** Open popup with fade+scale animation */
    function openSIHPopup() {
        if (popup.classList.contains('active')) return; // already open
        popup.classList.add('active');
        document.body.style.overflow = 'hidden';
    }

    /** Close popup and remember for this session */
    function closeSIHPopup() {
        popup.classList.remove('active');
        document.body.style.overflow = '';
        sessionStorage.setItem(SIH_SESSION_KEY, '1');
    }

    // --- Trigger: show popup after the loader finishes ---
    ...
    function tryShowPopup() {
        ...
        popupShown = true;
        // Small delay so the page feels settled before popup appears
        setTimeout(openSIHPopup, 600);
    }

    if (loader) {
        // Watch for loader being hidden (display:none or opacity:0)
        ...
        // Fallback: if the loader never hides (e.g. removed from DOM), show after 5s
        setTimeout(function () {
            observer.disconnect();
            tryShowPopup();
        }, 5000);
    } else {
        // No loader on the page — show popup immediately
        tryShowPopup();
    }
    ...
    // Close on × button
    if (closeBtn) {
        closeBtn.addEventListener('click', closeSIHPopup);
    }

    // Close on overlay (backdrop) click
    popup.addEventListener('click', function (e) {
        if (e.target === popup) closeSIHPopup();
    });
